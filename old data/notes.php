    ...
    <div class="result-wrapper">
        ...
        <div class="note-result">
            <figure class="note-img-hover">
                <div style="background-image: url('./database/<?php echo $r['filename'];?>.png');"
                    class="notes-file_preview"></div>
            </figure>
            <div class="detail" onload="render_thumbnail_notes('<?php echo $r['filename'];?>', '<?php echo $i;?>')">
                <h3 class="note-name"><?php echo $r['title']; ?></h3>
                <div class="description">
                    <p class="author-name">Author: <?php $name = $obj->get_author_name($r['author']); echo $name; ?></p>
                    <?php
                  if($r['verifier'] != 0) {
                ?>
                    <div class="verify">
                        ...
                        <p class="verify-name">
                            <?php 
                    $verifier = $obj->get_author_name($r['verifier']); 
                    echo $verifier; 
                  ?></p>
                    </div>
                    <?php
                ...
                <button class="view" id="<?php echo $r['id']; ?>" onclick="open_view_notes(this.id)">VIEW</button>
            </div>
        </div>
        <?php }}  else {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-secondary remove-note-btn float-left" id="delete-note-btn"
                    onclick="delete_note()" style="display: none;">Delete</button>
                <button class="btn btn-secondary remove-note-btn" id="remove-note-btn" onclick="remove_note()"
                    style="display: none;">Remove saved</button>
                <button class="btn btn-secondary save-note-btn" id="save-note-btn" onclick="save_note()"
